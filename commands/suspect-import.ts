...
const importGw = async (nameSet: Set<string>) => {
  ...
  for (let i = 0; i < 6; i++) {
    ...
    for (const entry of entries) {
      ...
      if (nameText.match(/DiFrancisco/)) {
        nameText = "Difrancesco, Kimberly";
      }

      if (nameText.match(/Kelly, Ryan/)) {
...
const linkType = (description: string, lastName?: string) => {
  ...
  switch (true) {
    ...
    case /Statement of Ofense/.test(description):
    case /Staterment of Offense /.test(description):
    ...
    case /Indicttment/.test(description):
      return "Indictment";
